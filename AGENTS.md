...
- Balance is always recomputed via `recalc_account_balance(account_id)` (a replay: iterate ops ordered by `(date, id)`, `balance = max(0.0, balance + amount)`). Deletion, trade editing and import all go through this replay. The `Старт` row cannot be deleted (`delete_operation` returns False).
...
- Menu is nested: main menu (`get_main_keyboard`) has compact rows «Аналитика» (`action_analytics` → stats/chart/excel, handled in `handlers/start.py`) and «Данные» (`action_data` → import/backup/reset). Stats, chart and excel screens return to the analytics menu, not the main one.
- Backup button appears only for `ADMIN_ID`, inside `get_data_keyboard` (`keyboards/inline.py`).
- Trade wizard (`handlers/trades.py`) supports step-by-step «Назад»: each step re-renders the previous prompt with a `wb_<step>` callback dispatched by `_BACK_HANDLERS` (no FSM gate). Confirmation screen's «Изменить» goes back to the date step. `get_wizard_keyboard(back_to)` builds `[◀️ Назад][🏠 Меню]`.
- Multiple handlers listen on `curr_` callbacks — they are gated by distinct FSM states (`DepositState`, `AccountState`, `SettingsState`), so the filters do not collide. Same trick for pickers that are reused for editing: `get_pairs_keyboard`/`get_side_keyboard`/`get_date_keyboard` take a `prefix` param (`sel_pair_`/`side`/`opdate` for add-trade vs `editpair_`/`editside`/`editdate` for history editing) — callbacks stay gated by their FSM state.
- Trade editing (`handlers/history.py`) covers date, pair, lot, side, amount, commission, note, risk. `get_trades`/`get_trade` rows end with `commission` (10 fields); `render_edit_detail` unpacks 10 values. `update_trade_commission` preserves gross (`amount + commission`) and recomputes the net result; only amount/commission editing triggers `recalc_account_balance`.
- Excel export (`utils/excel.py`) is currency-aware now: it reads the account currency for headers and number formats (don't hardcode `$`). Chart (`utils/chart.py`) uses matplotlib Agg and rotates x labels by hand (avoid `fig.autofmt_xdate` — it triggers the tick-copy path that recurses on matplotlib<numpy2). `generate_balance_chart_bytes(ops, currency, title=None, start_date=None)` recomputes the cumulative curve internally (does NOT trust `balance_after`), plots Win▲/Loss▼/Пополнение+/Вывод✕ markers and filters plotted points by `start_date` while keeping the pre-window balance.
...
- `utils/analytics.py`: `profit_factor` is `inf` when there are no losses (`format_stats_text` prints `∞`); stats also include `max_dd_pct`; `calculate_stats_by_pair` returns `[(pair, {n, wins, pl, winrate})]` sorted by count desc.
