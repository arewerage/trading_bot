...
def validate_amount(value: str, max_allowed: Optional[float] = None) -> Tuple[Optional[float], Optional[str]]:
    """
    ...
    try:
        ...
        return None, "Пожалуйста, введите корректное числовое значение (например, `50` или `-20`)."
...
def validate_lot(value: str) -> Tuple[Optional[float], Optional[str]]:
    ...
    try:
        ...
        return None, "Пожалуйста, введите корректный объём лота (например, `0.1` или `5`)."
...
def validate_trading_pair(value: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    if not value or not value.strip():
        return None, "Пара не может быть пустой. Введите название пары (например, `EURUSD`)."
    
    # Нормализуем: убираем спецсимволы и переводим в верхний регистр
    pair = value.strip().upper().replace("/", "").replace("-", "").replace(" ", "")
    
    if len(pair) < 3:
        return None, "Название пары слишком короткое (минимум 3 символа, например `EURUSD`)."
...
def validate_currency(value: str) -> Tuple[Optional[str], Optional[str]]:
    """
    Валидирует код валюты.
    
    Args:
        value: Строка с кодом валюты
        
    Returns:
        Кортеж (value, error_message)
    """
    allowed = {"USD", "EUR", "USDT", "GBP", "JPY"}
    currency = value.strip().upper()
    
    if currency not in allowed:
        return None, f"Валюта не поддерживается. Допустимые: {', '.join(allowed)}."
...
def validate_date_range(start_str: str, end_str: str) -> Tuple[Optional[Tuple[date, date]], Optional[str]]:
    """
    ...
    try:
        ...
        return None, "Неверный формат даты. Используйте ДД.ММ.ГГГГ (например, `01.01.2024`)."
...
def format_error(error_msg: str) -> str:
    """Оформляет сообщение об ошибке."""
    return f"⚠️ {error_msg}"


def format_warning(warning_msg: str) -> str:
    """Оформляет сообщение предупреждения."""
